...
# varsSum$ID = NULL
# write.csv(varsSum, 'varsSum.csv', row.names = F)
# vS <- read.csv('varsSum.csv')
# 
# svg("hist_Ac.svg")
# par(mfrow = c(2, 2))
# hist(vS$wc_bio10)
...
# # выбираем из тех, что коррелируют (-0.7, +0,7) условно
# # наименее подходящие нам
# 
...
# inputMxS$num <- sample(1:5, nrow(inputMxS), replace = T) # слишком мало точек
...
# dev.off()
# ######### вероятности ########
# avg_0 <- freq(MX_maps_rast > 0.75)
# avg_2_40 <- freq(map_245_40 > 0.75, useNA = "no")
# avg_2_60 <- freq(map_245_60 > 0.75, useNA = "no")
# avg_5_40 <- freq(map_585_40 > 0.75,  useNA = "no")
# avg_5_60 <- freq(map_585_60 > 0.75,  useNA = "no")
# low_0 <- freq(MX_maps_rast > 0.25)
# low_2_40 <- freq(map_245_40 > 0.25, useNA = "no")
# low_2_60 <- freq(map_245_60 > 0.25, useNA = "no")
# low_5_40 <- freq(map_585_40 > 0.25,  useNA = "no")
# low_5_60 <- freq(map_585_60 > 0.25,  useNA = "no")
# L_hal <- list(
#   Actual_a = avg_0[2, 3],
...
# 
# mean_act_f240 <- mean(values(act_f240), na.rm = TRUE)
# mean_act_f540 <- mean(values(act_f540), na.rm = TRUE)
# mean_act_f260 <- mean(values(act_f260), na.rm = TRUE)
# mean_act_f560 <- mean(values(act_f560), na.rm = TRUE)
# 
# mean1 <- c(mean_act_f240, mean_act_f540, mean_act_f260, mean_act_f560)
# 
...
# 
# ########### сравнение растров #########
# # Извлечение значений в векторы
# vals_act_f240 <- values(act_f240)
# vals_act_f540 <- values(act_f540)
# vals_act_f260 <- values(act_f260)
# vals_act_f560 <- values(act_f560)
# 
# # Удаление NA
# vals_act_f240 <- na.omit(vals_act_f240)
# vals_act_f540 <- na.omit(vals_act_f540)
# vals_act_f260 <- na.omit(vals_act_f260)
# vals_act_f560 <- na.omit(vals_act_f560)
# 
# df_anova <- data.frame(
#   value = c(vals_act_f240, vals_act_f540, vals_act_f260, vals_act_f560),
#   group = factor(rep(c('act_f240', 'act_f540', 'act_f260', 'act_f560'), 
#                      times=c(length(vals_act_f240), length(vals_act_f540),
#                              length(vals_act_f260), length(vals_act_f560))))
# )
# 
# # Однофакторный ANOVA
